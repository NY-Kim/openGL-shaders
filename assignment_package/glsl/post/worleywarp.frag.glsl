...
float WorleyNoise(vec2 uv) {
    uv *= mod289(u_Time) * 50 + 250; // Now the space is 10x10 instead of 1x1. Change this to any number you want.
    vec2 uvInt = floor(uv);
...
void main()
{
    ...
    for (int i = 0; i < 11; i++)
    {
        for (int j = 0; j < 11; j++)
        {
            ...
            if (dist < luminance || mod289(dist * u_Time + 0.01) < 0.5) {
                color = vec3(1, 1, 1);
            } else {
                color = vec3(0, 0, 0);
            }
